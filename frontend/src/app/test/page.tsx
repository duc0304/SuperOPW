...
const organizeContractsHierarchy = (contracts: ContractNode[]): ContractNode[] => {
  ...
  // Lọc ra chỉ các liability contracts ở cấp cao nhất
  const liabilityContracts = contracts.filter(c => c.type === 'liability');
  ...
  // Bước 1: Xây dựng maps và thêm liability contracts vào kết quả
  liabilityContracts.forEach(contract => {
    // Tạo bản sao để tránh các vấn đề tham chiếu
  ...
  // Bước 2: Tìm và gán các issue contracts vào liability parents
  contracts.forEach(contract => {
    if (contract.type === 'issue') {
      const issueCopy = { ...contract, children: [] };
      issueMap.set(contract.id, issueCopy);
      
      if (contract.oracleData?.ID) {
        idMap.set(contract.oracleData.ID, issueCopy);
      }
      
      // Tìm liability parent
  ...
  // Bước 3: Tìm và gán các card contracts vào issue hoặc liability parents
  contracts.forEach(contract => {
    if (contract.type === 'card') {
      const cardCopy = { ...contract, children: [] };
      
      // Kiểm tra parent là issue
      if (contract.oracleData?.ACNT_CONTRACT__OID) {
        ...
        if (parentIssue) {
          ...
          parentIssue.children.push(cardCopy);
        }
      } 
      // Kiểm tra parent là liability
        ...
        for (const [id, liabContract] of liabMap.entries()) {
          if (liabContract.oracleData?.ID === liabId) {
            liabContract.children = liabContract.children || [];
            liabContract.children.push(cardCopy);
            break;
          }
        }
      }
