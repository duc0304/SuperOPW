...
export default function ClientDetailsPage() {
  ...
  // Tham chiếu đến phần cuối trang
    ...
    const fetchClientDetails = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/api/oracle/clients/${params.id}`
        );
  ...
  const handleEnterEditMode = () => {
    ...
    // Cuộn xuống cuối trang sau khi chuyển sang chế độ edit
    setTimeout(() => {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/api/soap/editClient",
        {
          method: "PUT",
          headers: {
  ...
  // Header with action buttons - Xóa bỏ những nút khi ở chế độ Edit
  const renderActionBar = () => (
    <div className="flex items-center space-x-3 text-sm">
      {/* Chỉ để trống vì đã di chuyển nút xuống dưới */}
    </div>
  );
  ...
  return (
    <div className="p-4 pt-20 min-h-screen">
      <div className="container mx-auto max-w-7xl">
        ...
        {/* Grid layout for details - điều chỉnh padding để giảm chiều cao */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-indigo-100 dark:from-gray-800 dark:via-purple-900/20 dark:to-indigo-900/30 rounded-2xl shadow-xl border-2 border-purple-200/60 dark:border-purple-500/30 overflow-hidden transition-all duration-300 hover:shadow-xl">
            <div className="bg-gradient-to-r from-primary-700 via-primary-600 to-primary-400 dark:from-primary-900 dark:via-primary-800 dark:to-primary-600 px-6 py-3 flex items-center">
            ...
            <div className="p-6 space-y-4">
              {/* Client ID */}
              <div className="bg-white/60 dark:bg-gray-700/30 backdrop-blur-sm p-3 rounded-xl hover:bg-white/80 transition-all duration-300 border-2 border-indigo-300 dark:border-indigo-600/50 shadow-md">
                <label className="text-gray-500 dark:text-gray-400 block mb-1 text-sm font-semibold">
                  Client ID (Database)
                </label>
              ...
              {/* Company Name */}
              <div className="bg-white/60 dark:bg-gray-700/30 backdrop-blur-sm p-3 rounded-xl hover:bg-white/80 transition-all duration-300 border border-indigo-200 dark:border-indigo-700/30">
                <label className="text-gray-500 dark:text-gray-400 block mb-1 text-sm">
                  Company Name
                </label>
                {isEditMode ? (
                  <Input
                    ...
                    className="py-1 px-3 w-full bg-white/90 dark:bg-gray-700/70 border-purple-300 focus:border-primary-500"
                  />
                ) : (
              ...
              <div className="bg-white/60 dark:bg-gray-700/30 backdrop-blur-sm p-3 rounded-xl hover:bg-white/80 transition-all duration-300 border border-indigo-200 dark:border-indigo-700/30">
                ...
                {isEditMode ? (
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("FIRST_NAM", e.target.value)
                    }
                    className="py-1 px-3 w-full bg-white/90 dark:bg-gray-700/70 border-purple-300 focus:border-primary-500"
                  />
                ) : (
                  <p className="font-medium text-gray-900 dark:text-white">
                    {client.FIRST_NAM || "N/A"}
                  </p>
                )}
              </div>
              ...
              <div className="bg-white/60 dark:bg-gray-700/30 backdrop-blur-sm p-3 rounded-xl hover:bg-white/80 transition-all duration-300 border border-indigo-200 dark:border-indigo-700/30">
                ...
                {isEditMode ? (
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("LAST_NAM", e.target.value)
                    }
                    className="py-1 px-3 w-full bg-white/90 dark:bg-gray-700/70 border-purple-300 focus:border-primary-500"
                  />
                ) : (
              ...
              <div className="bg-white/60 dark:bg-gray-700/30 backdrop-blur-sm p-3 rounded-xl hover:bg-white/80 transition-all duration-300 border border-indigo-200 dark:border-indigo-700/30">
                ...
                {isEditMode ? (
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("FATHER_S_NAM", e.target.value)
                    }
                    className="py-1 px-3 w-full bg-white/90 dark:bg-gray-700/70 border-purple-300 focus:border-primary-500"
                  />
                ) : (
                  <p className="font-medium text-gray-900 dark:text-white">
                    {client.FATHER_S_NAM || "N/A"}
                  </p>
                )}
              </div>
              ...
              <div className="bg-white/60 dark:bg-gray-700/30 backdrop-blur-sm p-3 rounded-xl hover:bg-white/80 transition-all duration-300 border border-indigo-200 dark:border-indigo-700/30">
                ...
                {isEditMode ? (
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("PHONE_H", e.target.value)
                    }
                    className="py-1 px-3 w-full bg-white/90 dark:bg-gray-700/70 border-purple-300 focus:border-primary-500"
                  />
                ) : (
        ...
        {/* Tham chiếu đến phần cuối trang để cuộn xuống */}
